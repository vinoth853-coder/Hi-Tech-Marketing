...
function DeactivateUser({ details, onClose, onDeactivate }) {
  ...
  const handleSubmit = async () => {
    if (reason.trim() === '') {
      alertContent("Please provide a reason for deactivation.", "error");
      return;
    }
    try {
      const response = await API.put(`/user/deactivate_user/${details.id}`, { deactivate_reason: reason });
      if (response.status === 200) {
        onDeactivate(details.id, reason);
        alertContent('User successfully deactivated!', 'success');
        onClose();        
      } else {
        alertContent("Error! Couldn't deactivate the user.", "error");
      }
    } catch (error) {
      console.error('Error:', error);
      alertContent('An error occurred while deactivating the user. Please try again later.', "error");
    }
  }; 
  ...
  return (
    <div className="relative px-10 bg-secondary rounded-lg">
      ...
      <div className="mb-4">
        <label htmlFor="deactivationReason" className="block text-primary mb-1 font-semibold">Reason for Deactivation</label>
        <textarea
      ...
      <div className="flex justify-between mt-6 pb-10">
        ...
        <button type="button" onClick={handleSubmit} className="px-4 py-2 bg-primary text-secondary rounded-md hover:bg-gray-900">
          Submit
        </button>
      </div>
    </div>
